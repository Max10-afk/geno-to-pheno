...
inf_w = 1
freq_w = 1e-1
# rec_loss_fn = tf.keras.losses.CategoricalFocalCrossentropy(from_logits = False, axis = -2,
#             alpha = np.array([inf_w, freq_w, inf_w, inf_w, inf_w, freq_w, inf_w, inf_w, freq_w, inf_w, freq_w]))
# rec_loss_fn = tf.keras.losses.CategoricalCrossentropy(from_logits = False, axis = -2)
...
#         print("x_logits")
        ...
        # print(f"rec_loss.shape: {rec_loss.shape}")
        kl_scale = 1
        ...
        # rec_loss = tf.keras.losses.KLD(x_labels, x_softmax)
