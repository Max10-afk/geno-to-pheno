...
#        chr_pos = self.chr_embedding(chr_pos, training=training)
        chr_pos = chr_pos[0, ...] * 250.0 #self.chr_embedding(chr_pos, training=training)
        print(seq_pos[0, :].shape)
        print(chr_pos.shape)
        seq_pos = new_positional_encoding(seq_pos[0, :] + chr_pos, geno_x_embed[0].shape[3])
        ...
        att_geno_1 = self.ini_p_to_p_attention(
            tf.squeeze(diff_x) + seq_pos,
            tf.squeeze(geno_x[2]) + seq_pos,
            tf.squeeze(geno_x[2]),
            training=training,
        )
