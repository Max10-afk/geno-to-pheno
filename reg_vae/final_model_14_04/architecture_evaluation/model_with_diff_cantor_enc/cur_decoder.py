...
class decoder(Model):
    def __init__(self, latent_dim, depth = 1,**kwargs):
        ...
        self.final_dense = layers.EinsumDense("abc,cd->abd",
    ...
    def call(self, parents, embed_x, pos_data, training = False, return_activations = False):
        ...
        # pred = self.diff_to_z_att(
        #     parents_diff,
        #     geno_pred,
        #     training = training
        # )
        # pred = self.act_layer(pred)
        # pred = parents_diff + self.final_dense(pred, training = training)
