class encoder(Model):
    def __init__(self, latent_dim, width=1, **kwargs):
        ...
        self.diff_embedding = layers.Embedding(221, 8, name="ini_embedding")
    ...
    def call(self, p_genos, seq_pos, training=False, return_activations=False):
        act_tracker = {}
        ...
        # geno_x = tf.split(geno_x_embed, num_or_size_splits=geno_x_embed.shape[1], axis=1)
        pos_info = new_positional_encoding(seq_pos[0, :], geno_x_embed[0].shape[3])
        diff_x = cantor_pairing(tf.concat([geno_x_split[0], geno_x_split[1]], axis = 1))
        geno_x = [tf.squeeze(cur_geno) + pos_info for cur_geno in geno_x_embed]
        diff_x = self.diff_embedding(diff_x, training = training)
        #diff_x = (geno_x[0] - geno_x[1]) * geno_x[2]#
        diff_in = []
