class encoder(Model):
    def __init__(self, latent_dim, width=1, **kwargs):
        ...
        self.p_embedding = layers.Embedding(11, 8, name="enc_diff_embedding")
    ...
    def call(self, p_genos, pos_data, training=False, return_activations=False):
        ...
        geno_x = [tf.squeeze(cur_geno) for cur_geno in geno_x_embed] # + seq_pos + chr_pos
        p1_embed = self.p_embedding(tf.squeeze(geno_x_split[0]), training = training) # + seq_pos + chr_pos
        p2_embed = self.p_embedding(tf.squeeze(geno_x_split[1]), training = training) # + seq_pos + chr_pos
        diff_x = p1_embed - p2_embed
        diff_in = []
